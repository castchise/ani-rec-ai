// packages/jikan-client/src/index.ts
import type { Anime, UserAnimeEntry, WatchStatus } from "@ani-rec-ai/types";

const JIKAN_BASE = "https://api.jikan.moe/v4";

// Small sleep helper used for per-instance throttling
...
export class JikanClient {
  ...
  // The crawler uses BullMQ's limiter on top of this for process-level control.
  ...
  private async fetch<T>(path: string): Promise<T> {
    ...
    // 429 = rate limited — wait 1s and retry once
    if (res.status === 429) {
      await sleep(1000);
  ...
  // Filter by status: 'completed' | 'watching' | 'dropped' | 'onhold' | 'plantowatch'
  // Omit status to fetch all entries across all statuses.
  ...
  // Returns MAL usernames of a user's friends
  async getUserFriends(username: string): Promise<string[]> {
    try {
      ...
      return []; // friends list may be private or user may have none
    }
  }
}

// Jikan encodes watch status as numeric codes in animelist responses
...
// Jikan v4 response shapes (partial — extend as needed)
